...
const Airport: React.FC<AirportProps> = ({ PlaneClass = Plane, generateUniqueId = defaultGenerateUniqueId }) => {
  ...
  const land = (plane: InstanceType<typeof PlaneClass>): void => {
    try {
      if (hangerFull()) {
  ...
  const takeOff = (plane: InstanceType<typeof PlaneClass>): void => {
    try {
      if (!landed(plane)) {
  ...
  return (
    <Box p={4} data-testid="hanger-container">
      ...
      />
      <Button colorScheme="teal" onClick={() => handleLand(planeId)} m={2} disabled={hangerFull()}>Land Plane</Button>
